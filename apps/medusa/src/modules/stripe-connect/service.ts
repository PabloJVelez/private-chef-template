...
class StripeConnectProviderService extends AbstractPaymentProvider<StripeConnectProviderOptions> {
  ...
  constructor(
    ...
    this.config_ = {
      ...
      useStripeConnect,
      connectedAccountId:
        useStripeConnect && envAccountId.startsWith("acct_")
          ? envAccountId
          : "",
  ...
  async initiatePayment(
    ...
    const cartIdFromData =
      typeof dataObj?.cart_id === "string" ? dataObj.cart_id : undefined;
    const cartIdFromContext = (ctx?.cart_id ??
      ctx?.resource_id) as string | undefined;
    ...
    if (!this.config_.feePerUnitBased) {
      ...
      this.logger_.info(
        `${StripeConnectProviderService.LOG_PREFIX} [fee] mode=per_unit cart_id=${
          cartId ?? "none"
        } (from data=${!!cartIdFromData} context_keys=${
      ...
      if (cartId && typeof cartId === "string") {
        try {
          ...
          this.logger_.info(
            `${StripeConnectProviderService.LOG_PREFIX} [fee] cart lines count=${
              lines.length
    ...
    try {
      ...
      this.logger_.info(
        `${StripeConnectProviderService.LOG_PREFIX} Created PaymentIntent ${
          paymentIntent.id
        }: amount=${amountInCents} ${currency_code}` +
  ...
  async authorizePayment(
    ...
    try {
      ...
      this.logger_.error(
        `${StripeConnectProviderService.LOG_PREFIX} Failed to authorize payment: ${
  ...
  async capturePayment(
    ...
    try {
      ...
      this.logger_.error(
        `${StripeConnectProviderService.LOG_PREFIX} Failed to capture payment: ${
          (error as Error).message
        }`,
      );
      throw new MedusaError(
  ...
  async refundPayment(input: RefundPaymentInput): Promise<RefundPaymentOutput> {
    ...
    try {
      ...
      this.logger_.error(
        `${StripeConnectProviderService.LOG_PREFIX} Failed to refund payment: ${
          (error as Error).message
        }`,
      );
      throw new MedusaError(
  ...
  async cancelPayment(
    ...
    try {
      ...
      this.logger_.error(
        `${StripeConnectProviderService.LOG_PREFIX} Failed to cancel payment: ${
          (error as Error).message
        }`,
      );
      throw new MedusaError(
  ...
  async retrievePayment(
    ...
    try {
      ...
      this.logger_.error(
        `${StripeConnectProviderService.LOG_PREFIX} Failed to retrieve payment: ${
          (error as Error).message
        }`,
      );
      throw new MedusaError(
  ...
  async getPaymentStatus(
    ...
    try {
      ...
      this.logger_.warn(
        `${StripeConnectProviderService.LOG_PREFIX} Could not get payment status: ${
          (error as Error).message
        }`,
      );
  ...
  async updatePayment(
    ...
    try {
      ...
      this.logger_.error(
        `${StripeConnectProviderService.LOG_PREFIX} Failed to update payment: ${
          (error as Error).message
        }`,
      );
      throw new MedusaError(
  ...
  async getWebhookActionAndData(
    ...
    try {
      ...
      this.logger_.error(
        `${StripeConnectProviderService.LOG_PREFIX} Webhook processing error: ${
          (error as Error).message
        }`,
      );
      return {
